...
def run_experiment(model_class, model_name, batch_size, lr, epochs, patience,
                   momentum, dropout_p, optimizer_name):
    train_loader, val_loader, test_loader = load_cifar100(batch_size=batch_size)
    save_name = f"{model_name}_bs{batch_size}_lr{lr}_mom{momentum}_drop{dropout_p}_{optimizer_name}"
    ...
    if optimizer_name == "sgd":
        ...
        optimizer = Adam(model.params, lr=lr, weight_decay=0.0)
    else:
    ...
    trainer = Trainer(
        ...
        scheduler=None,
        experiment_name=f"optimizer_{model_name}",
...
if __name__ == "__main__":
    ...
    for model_class, model_name, batch_size, lr, momentum, dropout_p in model_settings:
        for optimizer_name in optimizers:
            if model_name == "ResNet20" and optimizer_name == "sgd":
                continue
            print(f"Training {model_name} with optimizer={optimizer_name}")
